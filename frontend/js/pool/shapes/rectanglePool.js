...
floor.userData.type = "floor";
  ...
  if (stepCount > 0) {
    ...
    // Apply the 300 mm set-in only when the small step itself is associated
    // with a curved local bench/wall segment. Do not use the full bench span
    // as the trigger, because a bench can be curved at one end while the step
    // is beside a straight/square-ended section at the other end.
    const curvedSideAnchor = benchSeatModeForAnchor && narrowLayout.placementPosition !== "center" && hasNonStraightWallSpan({
      ...narrowLayout,
      boundaryClip: {
        frame: stepWallFrame,
        distanceFromWall: getBench2Extension(params) * 0.5,
        alongCenter: narrowLayout.centerY
      }
    }, narrowLayout.width);
